...
class Combat {
  ...
  static makeAttack(attacker, target) {
    ...
    const baseAttackDamage = attacker.getMeta('attack-damage') || 1;
    const attackDescription = attacker.getMeta('attack-description') || 'HIT';
    let attackDamage = baseAttackDamage + (Random.inRange(1, baseAttackDamage) / 2);
    
    const damage = new Damage('stamina', attackDamage, attacker, attacker, { attackDescription: attackDescription});
